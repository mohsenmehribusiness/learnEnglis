...
@section('content')
<div class="row mt-5">
    <form id="formMultiWord" action="{{route('insert.multi.word.post')}}" method="post" class="form-inline">
        @csrf
    </form>
...
@section('script')
    <script>
        ...
        function addInputsForm(){
            ...
            <input type="text" class="form-control mx-1" id="word" name="`+idFormMultiWord+`[word]" placeholder="english" value="" required>
            <input type="text" class="form-control mx-1" id="persian" name="`+idFormMultiWord+`[persian]" placeholder="persian words..." required>
